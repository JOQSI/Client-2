  ...
  `,
    ...
    methods: {
        addTask() {
            ...
            if (this.card.tasks.length < 5 && this.newTask.trim()) {
                this.card.tasks.push({ text: this.newTask.trim(), completed: false });
                this.newTask = '';
                this.saveTasks();
            }
        },
    ...
    mounted() {
        ...
        const savedCard = savedCards.find(c => c.id === this.card.id);
...
new Vue({
    ...
    methods: {
        addCard() {
            const cardsInFirstColumn = this.cards.filter(card => !card.moved).length;
